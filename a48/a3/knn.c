...
void child_handler(Dataset *training, Dataset *testing, int K, 
	...
	//read the two values in from pipe
	...
	//printf("i am child: %d and my starting idx is: %d\n", getpid(), start_idx);
	...
	{
		if(knn_predict(testing, testing->images+i, K, fptr) == testing->labels[i]) //TODO: Arguments to fptr?
		{
			num_correct++;
		}
	}
	
	//write back num correct predictions to parent through p_out
...
double distance_cosine(Image *a, Image *b){
	...
	for(int i = 0; i< a->sx * a->sy; i++)
	{
		...
		bottom1 += (a->data[i]*a->data[i]);
		bottom2 += (b->data[i]*b->data[i]);
	}

    return (2/M_PI)*(acos(top/(sqrt(bottom1)*sqrt(bottom2))));
}
